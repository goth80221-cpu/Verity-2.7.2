...
do
    ...
    function WardrobeVisual.enable(id)
        for _,v in ipairs(WARDROBE.Extras) do if v.id==id then v.enabled=true break end end
        WardrobeVisual.render()
    end
    function WardrobeVisual.disable(id)
        for _,v in ipairs(WARDROBE.Extras) do if v.id==id then v.enabled=false break end end
        WardrobeVisual.render()
    end
    function WardrobeVisual.replace(oldId,newId)
        local found=false
        for _,v in ipairs(WARDROBE.Extras) do if v.id==oldId then v.id=newId found=true break end end
        if not found then WardrobeVisual.enable(newId) else WardrobeVisual.render() end
    end
    function WardrobeVisual.render()
        -- deterministic ownership/cleanup — no new Heartbeat/Maid
        for id,inst in pairs(accessoryInstances) do
            ...
            if not still then if inst.Parent then inst:Destroy() end accessoryInstances[id]=nil end
        end
        for _,v in ipairs(WARDROBE.Extras) do if v.enabled then
            local inst=accessoryInstances[v.id]
            if not inst then
                inst=Instance.new("Frame")
                inst.Name=v.id; inst.Size=UDim2.fromOffset(v.size.X, v.size.Y)
                inst.BackgroundTransparency=v.transparency; inst.ZIndex=v.zIndex
                inst.Parent=VerityCanvas or workspace
            ...
            inst.BackgroundTransparency=v.transparency
