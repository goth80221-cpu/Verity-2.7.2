...
-- Draggable/Locked persisted with backward-compatible defaults (older saved configs lack these)
local hubAppearance = {Theme="Theme_01", Scale=1.0, Position=nil, Transparency=0, Draggable=true, Locked=false}
-- Independent window state (no shared UIState.Locked)
local HubState = { Draggable=hubAppearance.Draggable, Locked=hubAppearance.Locked }
local VerityState = { Draggable=false, Locked=true }
local HubWindow = {}
function HubWindow.SetDraggable(b) HubState.Draggable = b and true or false end
function HubWindow.SetLocked(b) HubState.Locked = b and true or false end
local VerityWindow = {}
function VerityWindow.SetDraggable(b) VerityState.Draggable = b and true or false end
...
do
    ...
    if type(saved)=="table" and saved.Theme and THEMES[saved.Theme] then
        hubAppearance = saved
        -- backward-compatible defaults for Draggable/Locked (older saved configs lack these)
        if hubAppearance.Draggable == nil then hubAppearance.Draggable = true end
        if hubAppearance.Locked == nil then hubAppearance.Locked = false end
        HubState.Draggable = hubAppearance.Draggable
        HubState.Locked = hubAppearance.Locked
...
local shellCorner=corner(shell,10); shellStroke=stroke(shell,T.border,1)
...
do local drag=false; local sd,sp
    ...
    titleBar.InputBegan:Connect(function(i) if isMax or minimized or (not HubState.Draggable) or HubState.Locked then return end if i.UserInputType==Enum.UserInputType.MouseButton1 then drag=true sd=i.Position sp=Vector2.new(shell.Position.X.Offset,shell.Position.Y.Offset) end end)
...
do
    ...
    -- sub-section header helper (uppercase-dim + hairline)
    local function subHeader(text)
        new("Frame",{BackgroundColor3=T.border, Size=UDim2.new(1,0,0,1), BorderSizePixel=0, ZIndex=13, Parent=appCard})
        new("TextLabel",{BackgroundTransparency=1, Size=UDim2.new(1,0,0,18), Font=FONTB, Text=text, TextSize=10, TextColor3=T.subtext, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=13, Parent=appCard})
    end
    ...
    local summaryLabel = new("TextLabel",{BackgroundTransparency=1, Size=UDim2.new(1,0,0,18), Font=FONT, TextSize=10, TextColor3=T.dim, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=13, Parent=appCard})
    local function refreshSummary()
        ...
        local drag = HubState.Draggable and "Draggable" or "Fixed"
        local lock = HubState.Locked and "Locked" or "Unlocked"
        summaryLabel.Text = string.format("%s · %s · %s · %d%% · %s · %s", th.name, scaleToName(), pos, trans, drag, lock)
    end
    ...
    new("UIGridLayout",{CellSize=UDim2.fromOffset(122,30), CellPadding=UDim2.fromOffset(6,6), FillDirectionMaxCells=3, SortOrder=Enum.SortOrder.LayoutOrder, Parent=themeGrid})
    local themeBtns = {}
    ...
    for _,tid in ipairs(THEME_ORDER) do
        local th=THEMES[tid]
        local b=new("TextButton",{BackgroundColor3=th.colors.panel, Size=UDim2.fromOffset(122,30), Text=th.name, Font=FONT, TextSize=11, TextColor3=th.colors.text, AutoButtonColor=false, LayoutOrder=table.find(THEME_ORDER,tid) or 0, ZIndex=14, Parent=themeGrid}) corner(b,8)
        ...
        b.Activated:Connect(function()
            ...
            ThemeSystem.SetHubTheme(tid, true) refreshThemeButtons() refreshSummary()
            if tid=="Theme_07" then updateCustomEditor() end
            pushToast("Theme: "..th.name,"warn",1.4)
        end)
        themeBtns[tid]=b
        -- preview dot (right)
    ...
    local function applyPosition(preset)
        ...
        Settings:Set("hubAppearance", hubAppearance, "global"); Settings:Save("global")
        refreshPosition(); refreshSummary()
    ...
    local transRow = new("Frame",{BackgroundTransparency=1, Size=UDim2.new(1,0,0,22), ZIndex=13, Parent=appCard}) transRow.ToolTip = "Adjusts the Hub's overall transparency. Verity is unaffected."
    ...
    -- ===== HUB BEHAVIOR =====
    subHeader("HUB BEHAVIOR")
    local function behaviorRow(label, tip, value, onChange)
        local row = new("Frame",{BackgroundTransparency=1, Size=UDim2.new(1,0,0,28), ZIndex=13, Parent=appCard})
        new("TextLabel",{BackgroundTransparency=1, Size=UDim2.new(1,-60,1,0), Font=FONT, Text=label, TextSize=10, TextColor3=T.text, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=14, Parent=row})
        local tog = makeToggle(row, {value=value, onChange=onChange})
        tog.instance.Position = UDim2.new(1,-44,0.5,-10)
        if tip then tog.instance.ToolTip = tip end
        return tog
    end
    local dragTog = behaviorRow("Draggable", "Move the Hub by dragging its title bar.", hubAppearance.Draggable, function(v)
        HubWindow.SetDraggable(v)
        hubAppearance.Draggable = HubState.Draggable
        Settings:Set("hubAppearance", hubAppearance, "global"); Settings:Save("global")
        refreshSummary()
    end)
    local lockTog = behaviorRow("Lock position", "Pin the Hub in place.", hubAppearance.Locked, function(v)
        HubWindow.SetLocked(v)
        hubAppearance.Locked = HubState.Locked
        Settings:Set("hubAppearance", hubAppearance, "global"); Settings:Save("global")
        refreshSummary()
    end)
    ...
    -- ===== RESET =====
    subHeader("RESET")
    new("TextLabel",{BackgroundTransparency=1, Size=UDim2.new(1,0,0,16), Font=FONT, Text="Restore default appearance — Slate · Normal · Default · 0%", TextSize=10, TextColor3=T.dim, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=13, Parent=appCard})
    ...
    resetAppBtn.MouseButton1Click:Connect(function()
        hubAppearance = {Theme="Theme_01", CustomColors=nil, Scale=1.0, Position="Default", Transparency=0, Draggable=true, Locked=false}
        HubState.Draggable = true; HubState.Locked = false
        ...
        refreshThemeButtons(); refreshSize(); refreshPosition(); refreshSummary()
        dragTog.set(false); lockTog.set(false)
        transSlider.set(0)
        -- reset custom visibility/editor
