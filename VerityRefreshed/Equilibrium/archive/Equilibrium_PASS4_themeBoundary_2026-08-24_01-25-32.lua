...
local hubAppearance = {Theme="Theme_01", Scale=1.0, Position=nil, Transparency=0}
...
local canvas=new("CanvasGroup",{BackgroundTransparency=1, Size=UDim2.fromScale(1,1), GroupTransparency=hubAppearance.Transparency/100, ZIndex=11, Parent=shell})
-- HubRoot is the entire Hub visual root (shell) for theme scope; canvas is HubCanvasGroup for transparency grouping
local HubRoot = shell
local HubCanvasGroup = canvas
...
local hubIcon=new("Frame",{BackgroundColor3=T.panel, Size=offset(28,28), Position=offset(8,4), ZIndex=13, Parent=titleBar}) corner(hubIcon,14) stroke(hubIcon,T.border,1)
hubIcon:SetAttribute("ThemeRole","panel")
new("TextLabel",{BackgroundTransparency=1, Size=UDim2.fromScale(1,1), Font=FONTB, Text="EQ", TextSize=10, TextColor3=T.text, ZIndex=14, Parent=hubIcon})

new("TextLabel",{BackgroundTransparency=1, Size=UDim2.new(1,-190,1,0), Position=offset(42,0), Font=FONTB, Text="EQUILIBRIUM", TextSize=13, TextColor3=T.text, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=13, Parent=titleBar})
new("TextLabel",{BackgroundTransparency=1, Size=UDim2.new(1,-190,1,0), Position=offset(42,14), Font=FONT, Text="Slate • 070707 • Universal", TextSize=9, TextColor3=T.dim, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=13, Parent=titleBar})
...
do
    ...
    -- Filter + Export row (SchizHub polish: filter + organize)
    local ctrlRow = new("Frame",{BackgroundTransparency=1, Size=UDim2.new(1,0,0,28), ZIndex=13, Parent=tpCard}) hlist(ctrlRow,6)
    local filterBox = new("TextBox",{BackgroundColor3=T.bg, Size=UDim2.new(0.5,-3,0,28), Font=FONT, Text="", PlaceholderText="Filter…", PlaceholderColor3=T.dim, TextSize=11, TextColor3=T.text, ClearTextOnFocus=false, ZIndex=14, Parent=ctrlRow}) corner(filterBox,8) stroke(filterBox,T.border,1) pad(filterBox,0,0,8,8)
    local exportBtn = new("TextButton",{BackgroundColor3=T.panel, Size=UDim2.new(0.5,-3,0,28), Text="Export", Font=FONT, TextSize=11, TextColor3=T.text, AutoButtonColor=false, ZIndex=14, Parent=ctrlRow}) corner(exportBtn,8) stroke(exportBtn,T.border,1)
    exportBtn.Activated:Connect(function()
        local bank=getBank()
        local ok,json=pcall(HttpService.JSONEncode,HttpService,bank)
        if ok then
            local out="-- TP Bank PlaceId "..tostring(game.PlaceId).." count "..#bank.."\n-- game:GetService(\"HttpService\"):JSONDecode('' .. json .. '')\nreturn "..json
            if typeof(setclipboard)=="function" then pcall(setclipboard, out) pushToast("Exported "..#bank.." to clipboard","warn",1.6) else print(out) pushToast("Exported to console F9","warn",1.6) end
            _G.TPBankExport=bank
            _G.AssistantContext=_G.AssistantContext or {} _G.AssistantContext.tpBank=bank
            _G.AssistantContext.tpBankJson=json
        end
    end)
    -- Folder-aware prompt modal (single instance, parent screen) — fixed: store buttons in closure, not as Frame members
    ...
    local function ensurePrompt()
        ...
        local row = new("Frame",{BackgroundTransparency=1, Size=UDim2.new(1,0,0,28), ZIndex=52, Parent=box}) hlist(row,8)
    ...
    local gridLayout = new("UIGridLayout",{CellSize=UDim2.fromOffset(150,36), CellPadding=UDim2.fromOffset(6,6), FillDirectionMaxCells=2, SortOrder=Enum.SortOrder.LayoutOrder, Parent=gridHolder})
    local function refreshGrid()
        ...
        local bank=getBank()
        local filter=(filterBox.Text or ""):lower():gsub("^%s+",""):gsub("%s+$","")
        local filtered={}
        for i,e in ipairs(bank) do
            local hay=(e.name or ""):lower().." "..(e.folder or ""):lower()
            if filter=="" or hay:find(filter,1,true) then table.insert(filtered,{idx=i, entry=e}) end
        end
        for _,it in ipairs(filtered) do
            local idx,entry=it.idx,it.entry
            local card2=new("Frame",{BackgroundColor3=T.bg, Size=UDim2.fromOffset(150,36), LayoutOrder=idx, ZIndex=14, Parent=gridHolder}) corner(card2,8) stroke(card2,T.border,1) pad(card2,0,0,8,8)
            local titleTxt=entry.name or ("TP_"..idx) .. (entry.folder and entry.folder~="" and " ["..entry.folder.."]" or "")
            ...
            card2.InputBegan:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton2 then
                ...
                promptSaveConn=saveBtn.Activated:Connect(function()
                    ...
                    local newFolder=folderBox.Text:gsub("^%s+",""):gsub("%s+$","") entry.folder=newFolder
        ...
        if #filtered==0 and #getBank()>0 then
            new("TextLabel",{BackgroundTransparency=1, Size=UDim2.new(1,0,0,24), Font=FONT, Text="No matches — clear filter", TextSize=11, TextColor3=T.dim, TextXAlignment=Enum.TextXAlignment.Center, LayoutOrder=99, ZIndex=14, Parent=gridHolder})
        elseif #getBank()==0 then
            new("TextLabel",{BackgroundTransparency=1, Size=UDim2.new(1,0,0,24), Font=FONT, Text="No saves — Save Current Position", TextSize=11, TextColor3=T.dim, TextXAlignment=Enum.TextXAlignment.Center, LayoutOrder=99, ZIndex=14, Parent=gridHolder})
        end
    end
    filterBox:GetPropertyChangedSignal("Text"):Connect(refreshGrid)
...
-- Settings → Appearance (Pass 3 Hub Themes, Hub-only)
do
    local appCard = makeCard("Settings", "Appearance", "Theme • Size • Position • Transparency — Hub only")
    -- register for theme refresh
    registerThemed(appCard, "BackgroundColor3", "panel")
    -- Theme selector
    new("TextLabel",{BackgroundTransparency=1, Size=UDim2.new(1,0,0,14), Font=FONTB, Text="Theme", TextSize=11, TextColor3=T.subtext, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=13, Parent=appCard})
    local themeGrid = new("Frame",{BackgroundTransparency=1, Size=UDim2.new(1,0,0,0), AutomaticSize=Enum.AutomaticSize.Y, ZIndex=13, Parent=appCard})
    local tLayout = new("UIGridLayout",{CellSize=UDim2.fromOffset(86,28), CellPadding=UDim2.fromOffset(6,6), FillDirectionMaxCells=3, SortOrder=Enum.SortOrder.LayoutOrder, Parent=themeGrid})
    local themeBtns = {}
    ...
    for _,tid in ipairs(THEME_ORDER) do
        local th=THEMES[tid]
        local b=new("TextButton",{BackgroundColor3=th.colors.panel, Size=UDim2.fromOffset(86,28), Text=th.name, Font=FONT, TextSize=11, TextColor3=th.colors.text, AutoButtonColor=false, LayoutOrder=table.find(THEME_ORDER,tid) or 0, ZIndex=14, Parent=themeGrid}) corner(b,8)
        ...
        b.Activated:Connect(function()
            if tid=="Theme_07" then
                -- Custom = clone currently selected preset (not always Slate)
            ...
            ThemeSystem.SetHubTheme(tid, true) refreshThemeButtons() pushToast("Theme: "..th.name,"warn",1.4)
        end)
        themeBtns[tid]=b
        -- mini preview dot
        local dot=new("Frame",{BackgroundColor3=th.colors.accent, Size=UDim2.fromOffset(8,8), Position=UDim2.new(1,-12,0,4), AnchorPoint=Vector2.new(0,0), ZIndex=15, Parent=b}) corner(dot,4)
    end
    refreshThemeButtons()
    EVENTS.on("theme.changed", refreshThemeButtons)
    -- Hub Size
    new("TextLabel",{BackgroundTransparency=1, Size=UDim2.new(1,0,0,14), Font=FONTB, Text="Hub Size", TextSize=11, TextColor3=T.subtext, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=13, Parent=appCard})
    ...
    for _,pair in ipairs(sizes) do
        ...
        b.Activated:Connect(function()
            ...
            -- keep Verity 360x640 untouched
            Settings:Set("hubAppearance", hubAppearance, "global"); Settings:Save("global")
            refreshSize()
            pushToast("Hub Size: "..name,"warn",1.2)
        end)
        sizeBtns[name]=b
    end
    -- Hub Position
    new("TextLabel",{BackgroundTransparency=1, Size=UDim2.new(1,0,0,14), Font=FONTB, Text="Hub Position", TextSize=11, TextColor3=T.subtext, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=13, Parent=appCard})
    ...
    local function applyPosition(preset)
        ...
        Settings:Set("hubAppearance", hubAppearance, "global"); Settings:Save("global")
        pushToast("Hub Position: "..preset,"warn",1.2)
    end
    for _,p in ipairs(positions) do
        local b=new("TextButton",{BackgroundColor3=T.panel, Size=UDim2.new(0,64,0,28), Text=p, Font=FONT, TextSize=11, TextColor3=T.text, AutoButtonColor=false, ZIndex=14, Parent=posRow}) corner(b,8) stroke(b,T.border,1)
        b.Activated:Connect(function() applyPosition(p) end)
    end
    -- Transparency
    new("TextLabel",{BackgroundTransparency=1, Size=UDim2.new(1,0,0,14), Font=FONTB, Text="Transparency", TextSize=11, TextColor3=T.subtext, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=13, Parent=appCard})
    local transRow = new("Frame",{BackgroundTransparency=1, Size=UDim2.new(1,0,0,22), ZIndex=13, Parent=appCard})
    local transSlider = makeSlider(transRow, {label="Alpha", min=0, max=20, step=5, default=hubAppearance.Transparency or 0, onChange=function(v)
        hubAppearance.Transparency=v
        if HubCanvasGroup then HubCanvasGroup.GroupTransparency = v/100 end
        Settings:Set("hubAppearance", hubAppearance, "global"); Settings:Save("global")
    end})
    -- Custom hex editor (Hub only, 6-digit #RRGGBB, clones selected preset)
    ...
    new("TextLabel",{BackgroundTransparency=1, Size=UDim2.new(1,0,0,14), Font=FONTB, Text="Custom Colors — Paste #RRGGBB", TextSize=10, TextColor3=T.subtext, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=13, Parent=customFrame})
    local customTokens = {"bg","panel","panel2","border","text","subtext","dim","accent","accentDim","on","off","warn","danger","success"}
    local customRows = {}
    for _,tok in ipairs(customTokens) do
        local row = new("Frame",{BackgroundTransparency=1, Size=UDim2.new(1,0,0,22), ZIndex=13, Parent=customFrame}) hlist(row,6)
        new("TextLabel",{BackgroundTransparency=1, Size=UDim2.new(0,70,1,0), Font=FONT, Text=tok, TextSize=10, TextColor3=T.subtext, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=14, Parent=row})
        local swatch = new("Frame",{BackgroundColor3=T[tok] or Color3.fromHex("070707"), Size=UDim2.fromOffset(16,16), ZIndex=14, Parent=row}) corner(swatch,3) stroke(swatch,T.border,1)
        local hexBox = new("TextBox",{BackgroundColor3=T.bg, Size=UDim2.new(1,-100,1,0), Font=FONT, Text=(hubAppearance.CustomColors and hubAppearance.CustomColors[tok] or string.format("#%02X%02X%02X", math.floor((T[tok] or Color3.new(0,0,0)).R*255+0.5), math.floor((T[tok] or Color3.new(0,0,0)).G*255+0.5), math.floor((T[tok] or Color3.new(0,0,0)).B*255+0.5))), PlaceholderText="#RRGGBB", PlaceholderColor3=T.dim, TextSize=10, TextColor3=T.text, ClearTextOnFocus=false, ZIndex=14, Parent=row}) corner(hexBox,6) stroke(hexBox,T.border,1) pad(hexBox,0,0,6,6)
        local function applyHex()
            local txt = hexBox.Text:gsub("%s+","")
            if not txt:match("^#%x%x%x%x%x%x$") then pushToast("Invalid color — use #RRGGBB","warn",1.2) hexBox.Text = hubAppearance.CustomColors and hubAppearance.CustomColors[tok] or string.format("#%02X%02X%02X", math.floor((T[tok] or Color3.new(0,0,0)).R*255+0.5), math.floor((T[tok] or Color3.new(0,0,0)).G*255+0.5), math.floor((T[tok] or Color3.new(0,0,0)).B*255+0.5)) return end
            hubAppearance.CustomColors = hubAppearance.CustomColors or {}
            hubAppearance.CustomColors[tok]=txt
            swatch.BackgroundColor3 = Color3.fromHex(txt:gsub("#",""))
            ThemeSystem.SetHubTheme("Theme_07", true)
        end
        hexBox.FocusLost:Connect(function(enter) if enter then applyHex() end end)
        customRows[tok]={swatch=swatch, box=hexBox}
    end
    ...
    resetCustomBtn.Activated:Connect(function()
        local srcId = hubAppearance.Theme ~= "Theme_07" and hubAppearance.Theme or "Theme_01"
        local src = THEMES[srcId] or THEMES.Theme_01
        local clone={}
        for k,col in pairs(src.colors) do clone[k]=string.format("#%02X%02X%02X", math.floor(col.R*255+0.5), math.floor(col.G*255+0.5), math.floor(col.B*255+0.5)) end
        hubAppearance.CustomColors = clone
        ThemeSystem.SetHubTheme("Theme_07", true)
        for tok,row in pairs(customRows) do
            local hex = clone[tok] or "#070707"
            row.box.Text = hex
            row.swatch.BackgroundColor3 = Color3.fromHex(hex:gsub("#",""))
        end
        pushToast("Custom reset to "..src.name,"warn",1.2)
    end)
    local function updateCustomVisibility(id)
        customFrame.Visible = (id=="Theme_07")
        if id=="Theme_07" and hubAppearance.CustomColors then
            for tok,row in pairs(customRows) do
                local hex = hubAppearance.CustomColors[tok]
                if hex then
                    row.box.Text = hex
                    pcall(function() row.swatch.BackgroundColor3 = Color3.fromHex(hex:gsub("#","")) end)
                end
            end
        end
    end
    EVENTS.on("theme.changed", updateCustomVisibility)
    -- Reset Appearance button
    local resetAppBtn = new("TextButton",{BackgroundColor3=T.warn, Size=UDim2.new(1,0,0,24), Text="Reset Appearance — Slate / Normal / Default / 0", Font=FONTB, TextSize=11, TextColor3=Color3.new(1,1,1), AutoButtonColor=false, ZIndex=14, Parent=appCard}) corner(resetAppBtn,8)
    resetAppBtn.Activated:Connect(function()
        hubAppearance = {Theme="Theme_01", CustomColors=nil, Scale=1.0, Position="Default", Transparency=0}
