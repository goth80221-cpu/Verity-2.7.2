...
local hubAppearance = {Theme="Theme_01", Scale=1.0, Position=nil, Transparency=0, PositionMode="Draggable"}
-- Independent window state (no shared UIState.Locked) — single Mode
local HubState = { Mode="Draggable" }
local VerityState = { Locked=true } -- canonical: draggable = not Locked
local HubWindow = {}
...
do
    ...
    local filterBox = new("TextBox",{BackgroundColor3=T.bg, Size=UDim2.new(0.5,-3,0,28), Font=FONT, Text="", PlaceholderText="Search positions...", PlaceholderColor3=T.dim, TextSize=11, TextColor3=T.text, ClearTextOnFocus=false, ZIndex=14, Parent=ctrlRow}) corner(filterBox,8) stroke(filterBox,T.border,1) pad(filterBox,0,0,8,8)
    -- SAVED POSITIONS header
    new("TextLabel",{BackgroundTransparency=1, Size=UDim2.new(1,0,0,16), Font=FONTB, Text="SAVED POSITIONS", TextSize=9, TextColor3=T.subtext, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=13, Parent=tpCard})
    new("Frame",{BackgroundColor3=T.border, Size=UDim2.new(1,0,0,1), BorderSizePixel=0, ZIndex=13, Parent=tpCard})
...
do
    ...
    for _,tid in ipairs(THEME_ORDER) do
        ...
        b.Activated:Connect(function()
            ...
            ThemeSystem.SetHubTheme(tid, true) refreshThemeButtons() refreshSummary() RefreshAppearancePreview()
    ...
    -- ===== HUB BEHAVIOR (single Mode) =====
    subHeader("HUB BEHAVIOR")
    new("TextLabel",{BackgroundTransparency=1, Size=UDim2.new(1,0,0,14), Font=FONT, Text="Hub Position", TextSize=10, TextColor3=T.text, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=13, Parent=appCard, TextWrapped=true})
    ...
    resetAppBtn.MouseButton1Click:Connect(function()
        hubAppearance = {Theme="Theme_01", CustomColors=nil, Scale=1.0, Position="Default", Transparency=0, PositionMode="Draggable"}
...
local EmotionState = { Name="Neutral", Target="Neutral", Current=nil, Elapsed=0, Duration=0.20, From=nil }
EmotionState.Current = EMOTIONS.Neutral
EmotionState.From = EMOTIONS.Neutral
...
-- layered emotion interpolation (priority Emotion 10, Reaction 30 overrides)
local function UpdateEmotion(dt)
    if EmotionState.Elapsed < EmotionState.Duration then
        ...
        -- ease quad out
        p = 1 - (1-p)*(1-p)
        local from = EmotionState.From or EMOTIONS.Neutral
        local to = EMOTIONS[EmotionState.Target] or EMOTIONS.Neutral
        -- blush lerp
        local blush = from.Blush + (to.Blush - from.Blush) * p
        if Layers.blushL then Layers.blushL.BackgroundTransparency = 1 - blush end
        if Layers.blushR then Layers.blushR.BackgroundTransparency = 1 - blush end
        -- head tilt lerp
        local tilt = from.HeadTilt + (to.HeadTilt - from.HeadTilt) * p
        if Anchors.HeadAnchor and animationState.Current~="Reaction" then
            -- Reaction priority 30 > Emotion 10 for tilt, so skip if reacting
            Anchors.HeadAnchor.Rotation = tilt
        end
        if p >= 1 then EmotionState.Current = EMOTIONS[EmotionState.Target] EmotionState.From = EmotionState.Current end
    end
end

-- separate persistent Emotion vs temporary Reaction: Reaction restores EmotionState
local _origPlayReaction = PlayReaction
PlayReaction = function(name)
    local prevEmotion = EmotionState.Name
    _origPlayReaction(name)
    -- after reaction duration, original restores PROFILE.Expression but EmotionState persists
    task.delay(ANIMATION_TIMING.Reaction[name] or ANIMATION_TIMING.Reaction.DefaultDuration, function()
        if EmotionState.Name == prevEmotion then return end
        -- ensure EmotionState not overwritten by Reaction's expression restore
        VerityVisual.ApplyEmotion(EmotionState.Name)
    end)
end
...
-- PASS 5: Emotion declarative API — only changes state, controller updates visual
function Verity.SetEmotion(name, instant)
    if not name then name="Neutral" end
    if not EMOTIONS[name] then name="Neutral" end
    EmotionState.From = EMOTIONS[EmotionState.Name] or EMOTIONS.Neutral
    EmotionState.Name = name
    EmotionState.Target = name
    EmotionState.Elapsed = 0
    EmotionState.Duration = instant and 0 or (0.15 + math.random()*0.15)
    if EmotionState.Duration==0 and VerityVisual then VerityVisual.ApplyEmotion(name) end
end
function Verity.SetTalking(on)
    if on then animationState.Talking=true; animationTimers.Talk=0; talkIdx=1
    else animationState.Talking=false end
end
-- VerityVisual — sole GUI owner (Assistant -> Verity API -> Emotion/Reaction/Talking -> Animation -> Visual)
local VerityVisual = {}
function VerityVisual.ApplyEmotion(name)
    local e=EMOTIONS[name] or EMOTIONS.Neutral
    -- Blush
    if Layers.blushL then Layers.blushL.BackgroundTransparency = 1 - (e.Blush or 0) end
    if Layers.blushR then Layers.blushR.BackgroundTransparency = 1 - (e.Blush or 0) end
    -- Head tilt handled by AnimationController
end
